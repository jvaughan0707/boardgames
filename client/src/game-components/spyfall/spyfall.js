    ...
    return (
      <div id="spyfall-container">
        <div id="players">
          {
            game.players.map(p => (
              <div key={p.userId} className={`${game.state.spies && p.state.spy ? 'spy' : ''} player`}>
                <div className="player-inner">
                  <h3>{p.displayName}</h3>
                  {
                    ...
                    <button title="You can only do this once per round"
                      ...
                      disabled={!user.state.canNominate || game.state.paused}>Nominate</button>
                  }
        ...
        <div id="info">
          <Timer paused={game.state.paused} initial={game.state.totalTime} length={game.state.timerLength} from={game.state.timerFrom}></Timer>
          ...
          }
          {
            ...
            <div>
              ...
              <div className="votes">
                Votes:
                {game.players.map(p => p.userId === game.state.nominatedPlayer.userId ?
                null :
                p.state.vote === null ?
                  <FontAwesomeIcon icon="question" title={p.displayName} /> :
                  p.state.vote ?
                    <FontAwesomeIcon icon="check" title={p.displayName} /> :
                    <FontAwesomeIcon icon="times" title={p.displayName} />
              )}
              </div>
        ...
        <div id="locations">
          {
            Object.keys(locations).map(id => {
              ...
              return (
                <div key={id} className="location" style={{backgroundImage: `url(${img})`}}
